...
import { castAddGoalData } from "../utils/Helper";
...
const AddGoal = () => {
  ...
  const [goalPrevImg, setGoalPrevImg] = useState(null);
  ...
  const handleGoalCreate = async (formData) => {
    try {
      startGloablLoading();
      console.log(formData);
      if (!formData?.hasOwnProperty("goalType") || formData.goalType === null) {
        setError("goalType", {
          type: String,
      ...
      if (
        !formData?.hasOwnProperty(
          "goalCategory" || formData.goalCategory === "Category"
        )
      ) {
        setError("goalCategory", {
          type: String,
          message: "Goal category is required",
        });
        return;
      }

      const data = castAddGoalData(formData);
      const res = await fighterApi.createGoals(token, data);

      if (res.status === 200) {
        toast.success(res?.data?.message);
        navigate("/fighter/goal-detail", {
          state: { goalId: res?.data?.data?._id },
        });
      }
      // console.log(res);
    } catch (e) {
  ...
  useEffect(() => {
    if (watch("image") && watch("image").length > 0) {
      setGoalPrevImg(URL.createObjectURL(watch("image")[0]));
    }
  }, [watch("image")]);
            ...
            >
              ...
              <div className="form-group text-left mb-2">
                ...
                <input
                  ...
                  className={`form-control ${
                    errors?.goalName ? "error-border-profile" : ""
                  }`}
                  placeholder="Enter item name"
                  {...register("goalName", {
                    required: {
                      value: true,
                      message: "Gaol name is required",
                    },
                    pattern: {
                      value: /^[A-Za-z\s,'-]+$/,
              ...
              <div className="form-group text-left mb-2">
                ...
                <input
                  ...
                  id="goalprice"
                  className={`form-control ${
                    errors?.goalPrice ? "error-border-profile" : ""
                  }`}
                  placeholder="CAD $"
                  {...register("goalPrice", {
                    required: {
                      value: true,
                      message: "Goal price is required",
                    },
                    pattern: {
                      value: /^(0|[1-9][0-9]*)$/,
              ...
              <div className="form-group text-left mb-2">
                ...
                <div className="goal-file-wrapper">
                  <input
                    type="file"
                    id="goalimgupload"
                  ...
                  />
                  <label
                    htmlFor="goalimgupload"
                    className={`goal-img-upload ${
                      errors?.image ? "error-border-profile" : ""
                    }`}
                  >
                    {goalPrevImg && (
                      <img
                        src={goalPrevImg}
                        alt=""
                        className="w-100 h-100 goal-preve-image"
                      />
                    )}
                    <div className="position-absolute">
                      ...
                      <span className="d-block">Upload Image Here</span>
                    </div>
                  </label>
                </div>
                <p className="profile-error-message">
              ...
              <GoalType register={register} watch={watch} errors={errors} />
              ...
              <div className="f-submit mt-5">
                ...
                >
                  {globalLoading ? "loading..." : "Add Goal"}
                </button>
              </div>
            </form>
          </div>
